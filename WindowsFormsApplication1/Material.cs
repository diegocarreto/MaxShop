...
{
    public partial class Material : FormBase
    {
        #region Properties

        private posb.Material Entity { get; set; }
        ...
        {
            if (this.Confirm("¿Realmente deseas eliminar la propiedad [" + this.EntityName + "]?"))
            {
                ...
                if (this.Entity.Delete())
                {
                    ...
                    this.Alert("Ocurrió un error al intentar eliminar la propiedad [" + this.EntityName + "]", eForm.TypeError.Error);
            }
        }
        ...
        {
            MaterialEdit MeasureEdit = new MaterialEdit(Id);

            MeasureEdit.Result += new MaterialEdit.Communication(Result);
