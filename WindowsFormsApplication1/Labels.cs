...
{
    public partial class Labels : FormBase
    {
        #region Properties

        private posb.Label Entity { get; set; }
        ...
        {
            if (this.Confirm("¿Realmente deseas eliminar la etiqueta [" + this.EntityName + "]?"))
            {
                ...
                if (this.Entity.Delete())
                {
                    ...
                    this.Alert("Ocurrió un error al intentar eliminar la etiqueta [" + this.EntityName + "]", eForm.TypeError.Error);
            }
        }
        ...
        {
            LabelsEdit MeasureEdit = new LabelsEdit(Id);

            MeasureEdit.Result += new LabelsEdit.Communication(Result);
