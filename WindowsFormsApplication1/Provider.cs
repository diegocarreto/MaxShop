...
{
    public partial class Provider : FormBase
    {
        #region Properties

        private posb.Brand Entity { get; set; }
        ...
        {
            if (this.Confirm("¿Realmente deseas eliminar la marca [" + this.EntityName + "]?"))
            {
                ...
                if (this.Entity.Delete())
                {
                    ...
                    this.Alert("Ocurrió un error al intentar eliminar la marca [" + this.EntityName + "]", eForm.TypeError.Error);
            }
        }
        ...
        {
            BrandEdit MeasureEdit = new BrandEdit(Id);

            MeasureEdit.Result += new BrandEdit.Communication(Result);
